...
class RewardHParams:
    ...
    trained_model: Optional[str] = "sentiment-analysis:lvwerra/distilbert-imdb"
...
if __name__ == "__main__":
    ...
    task, model_name = args.rewards.trained_model.split(":")
    sent_kwargs = {"return_all_scores": True, "function_to_apply": "none", "batch_size": 16}

    pipeline_device = device
    if accelerator.num_processes == 1:
        pipeline_device = 0 if torch.cuda.is_available() else "cpu"  # to avoid a `pipeline` bug
    sentiment_pipe = pipeline(task, model=model_name, device=pipeline_device)
    ...
    for update in range(1, args.ppo.num_updates + 1):
        ...
        with torch.no_grad():
            ...
            postprocessed_query_responses_text = tokenizer.batch_decode(
                postprocessed_query_responses, skip_special_tokens=True
            )
            pipe_outputs = sentiment_pipe(postprocessed_query_responses_text, **sent_kwargs)
            scores = torch.stack([torch.tensor(output[1]["score"]) for output in pipe_outputs]).to(device)
