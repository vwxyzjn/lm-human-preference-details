...
def train(args: Args):
    ...
    reward_model = AutoModelForCausalLMWithRewardHead(AutoModelForCausalLM.from_pretrained(args.base_model, trust_remote_code=True)).to(second_device)
    ...
    ref_policy = AutoModelForCausalLMWithScalarHead(AutoModelForCausalLM.from_pretrained(args.base_model, trust_remote_code=True)).to(second_device)
    policy = AutoModelForCausalLMWithScalarHead(AutoModelForCausalLM.from_pretrained(args.base_model, trust_remote_code=True)).to(device)
    ...
    for update in range(1, args.ppo.num_updates + 1):
        ...
        with torch.no_grad():
            ...
            queries_2nd_device = queries.to(second_device)
            query_responses_2nd_device = query_responses.to(second_device)
            responses_2nd_device = responses.to(second_device)

            ref_output, _ = forward(ref_policy, query_responses_2nd_device, tokenizer)
            ref_logits = ref_output.logits[:,context_length-1:-1]
            ref_logits /= args.task.temperature
            ref_all_logprobs = F.log_softmax(ref_logits, dim=-1)
            ref_logprobs = torch.gather(ref_all_logprobs, 2, responses_2nd_device.unsqueeze(-1)).squeeze(-1)
            ...
            truncate_token_mask = (responses_2nd_device == args.task.truncate_token)
            ...
            postprocessed_responses = torch.where(truncate_mask, torch.full_like(responses_2nd_device, tokenizer.pad_token_id), responses_2nd_device)
            ...
            postprocessed_query_responses = torch.cat((queries_2nd_device, postprocessed_responses), 1).to(second_device)
            ...
            filter_mask = torch.any(matches_token, dim=-1).to(second_device)
            scores = torch.where(filter_mask, scores, torch.full_like(scores, args.task.penalty_reward_value)).to(device)
            del matches_token, filter_mask; torch.cuda.empty_cache()

            # 4. compute rewards
            kl = logprobs - ref_logprobs.to(device)
