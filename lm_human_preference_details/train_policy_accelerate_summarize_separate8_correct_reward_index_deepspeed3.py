...
if __name__ == "__main__":
    ...
    if args.optimizer == "tf_adam":
        ...
        if args.deepspeed:
            ...
            optimizer = DeepSpeedCPUAdam(model.parameters(), lr=args.ppo.lr, eps=args.ppo.eps)
        else:
    ...
    if args.deepspeed:
        ...
        deepspeed_plugin.deepspeed_config["checkpoint"] = {"use_node_local_storage": True}
        ...
        config_kwargs = {
            ...
            "zero_optimization": {
                "stage": 3,
                "offload_param": {"device": deepspeed_plugin.offload_param_device},
                "stage3_prefetch_bucket_size": 0,
                "stage3_max_live_parameters": 0,
                "stage3_max_reuse_distance": 0,
            },
        }
        accelerator.print(config_kwargs)
        reward_model, *_ = deepspeed.initialize(model=reward_model, config=config_kwargs)
        reward_model.eval()
        ref_policy, *_ = deepspeed.initialize(model=ref_policy, config=config_kwargs)
        ref_policy.eval()
    ...
    with torch.no_grad():
        ...
        accelerator.print(f"==={queries.shape=}, {queries.dtype}")
        accelerator.print(f"==={sample_validation_query_reference_responses.shape=}, {sample_validation_query_reference_responses.dtype}")
