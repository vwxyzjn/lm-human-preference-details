        ...
        for start in range(0, len_labels, args.batch_size):
            ...
            for micro_batch_start in range(0, args.local_batch_size, args.local_micro_batch_size):
                ...
                for i in range(args.labels.num_labels):
                    ...
                    score, reward = get_reward_complete(reward_model, query_responses, args)
...
def train(args: Args):
    ...
    reward_model.load_state_dict(torch.load("models/gpt2-medium-rm/pytorch_model.bin", map_location=device))
        ...
        for (epoch_global_step, start) in enumerate(range(0, num_train, args.batch_size)):
            ...
            for micro_batch_start in range(0, args.local_batch_size, args.local_micro_batch_size):
                with accelerator.accumulate(reward_model):
                    ...
                    predicted_reward = predicted_reward.view(
                        -1, len(mb_responses)
                    )  # TODO check shape for no gradienta ccumulation steps
            ...
            print("train/reward_preferred", accelerator.gather(reward_preferreds))
            print("train/reward_rejected", accelerator.gather(reward_rejecteds))
            breakpoint()
