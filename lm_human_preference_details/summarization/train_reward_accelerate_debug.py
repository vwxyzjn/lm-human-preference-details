...
def format_query(query_prefix_tokens, query, query_suffix_tokens):
    query_prefix_tokens_tiled = query_prefix_tokens.unsqueeze(0).repeat(query.shape[0], 1).to(query.device)
    query_suffix_tokens_tiled = query_suffix_tokens.unsqueeze(0).repeat(query.shape[0], 1).to(query.device)
    return torch.cat((query_prefix_tokens_tiled, query, query_suffix_tokens_tiled), dim=1)
