...
def train(args: Args):
    ...
    num_updates = args.labels.num_train // args.batch_size
    ...
    reward_model.lm_backbone.gradient_checkpointing_enable()
    ...
    # pprint(process_query_data(dataset[0]))
        ...
        return {
            **process_query(x["info"], encoder=tokenizer, hparams=patch_h),
            "response0_token": tokenizer.encode(
                f" {x['summaries'][0]['text']}<|endoftext|>",
                padding="max_length",
                max_length=args.task.response_length,
                truncation=True,
            ),
            "response1_token": tokenizer.encode(
                f" {x['summaries'][1]['text']}<|endoftext|>",
                padding="max_length",
                max_length=args.task.response_length,
                truncation=True,
            ),
    ...
    dev_validation_label = dev_validation_label.map(process_response_data)
    eval_validation_label = eval_validation_label.map(process_response_data)
    ...
    for (global_step, start) in enumerate(range(0, args.labels.num_train, args.batch_size)):
        ...
        for micro_batch_start in range(0, args.local_batch_size, args.local_micro_batch_size):
            with accelerator.accumulate(reward_model):
                ...
                predicted_rewards, score_all = get_reward_complete(reward_model, query_responses, tokenizer)
                breakpoint()

                predicted_rewards = predicted_rewards.view(len(mb_responses), -1)
                ...
                if args.logsigmoid:
                    ...
                    loss = F.cross_entropy(predicted_rewards, mb_best)
        ...
        if global_step == num_updates - 1:  # first and last update
