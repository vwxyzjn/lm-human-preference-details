...
def train(args: Args):
    ...
    for update in range(1, args.ppo.num_updates + 1):
        ...
        with torch.no_grad():
            ...
            output4, _ = forward(policy, query_responses, tokenizer)
            logits4 = output4.logits[:, context_length - 1 : -1]
            logits4 /= args.task.temperature
            all_logprobs4 = F.log_softmax(logits4, dim=-1)
            logprobs4 = torch.gather(all_logprobs4, 2, responses.unsqueeze(-1)).squeeze(-1)
        ...
        for ppo_epoch_idx in range(args.ppo.noptepochs):
            ...
            for mini_batch_start in range(0, args.ppo.local_batch_size, args.ppo.local_mini_batch_size):
                ...
                for micro_batch_start in range(0, args.ppo.local_mini_batch_size, args.ppo.local_micro_batch_size):
                    ...
                    output2, vpred_temp = forward(policy, mb_query_responses, tokenizer)
                    logits2 = output2.logits[:, context_length - 1 : -1]
                    logits2 /= args.task.temperature
                    new_all_logprobs2 = F.log_softmax(logits2, dim=-1)
                    new_logprobs2 = torch.gather(new_all_logprobs2, 2, mb_responses.unsqueeze(-1)).squeeze(-1)
                        ...
                        pprint(
                            {
                                ...
                                "new_logprobs2": new_logprobs2,
                                "mb_logprobs": mb_logprobs,
                                "mb_logprobs2": logprobs4[micro_batch_inds],
                            }
                        )
