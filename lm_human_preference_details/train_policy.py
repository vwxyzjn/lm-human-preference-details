    ...
    for update in range(1, args.ppo.num_updates + 1):
        ...
        for ppo_epoch_idx in range(args.ppo.noptepochs):
            ...
            for mb_start in range(0, args.ppo.local_batch_size, args.ppo.local_mini_batch_size):
                ...
                vpred = policy.scalar_head(output.hidden_states[-1])[:,context_length-1:-1].squeeze(-1)
        ...
        writer.add_scalar("ppo/loss/norm", norm.item(), update)
        ...
        writer.add_scalar("ppo/val/ratio", ratio.mean().item(), update)
